      ...
      for (int i=0; i<numDOFs; i++)
	{
	  ...
	  if (limited_hnp1[i] < -1E-14 && dt < 1.0)
	  ...
	  else
	    {
	      ...
	      //limited_hunp1[i] *= (limited_hnp1[i] < hReg[i] ? 0. : 1.);
	      //limited_hvnp1[i] *= (limited_hnp1[i] < hReg[i] ? 0. : 1.);
		  ...
		  if (i != j) // This is not necessary. See formula for ith_dissipative_terms
		    {
		      ...
		      //					      hi,hui,hvi,
		      //					      hj,huj,hvj,
		      //					      hEps,hEps,true)*cij_norm, //hEps
										       //		maxWaveSpeedSharpInitialGuess(g,nxji,nyji, 
		      //hj,huj,hvj,
		      //					      hi,hui,hvi,
		      //					      hEps,hEps,true)*cji_norm); //hEps
		      //  std::cout << dLowij << std::endl;
		      //  abort();
		      //}
